...
const getAllVisitSchedules = asyncHandler(async (req, res, next) => {
  ...
  res.status(200).json({
    ...new ApiResponse(
      visitSchedules,
      'All visit schedules retrieved successfully'
    ),
...
const getVisitSchedule = asyncHandler(async (req, res, next) => {
  ...
  res
    .status(200)
    .json(
      new ApiResponse(visitSchedule, 'Visit schedule retrieved successfully')
    );
});
...
const createVisitSchedule = asyncHandler(async (req, res, next) => {
  ...
  res
    .status(201)
    .json(
      new ApiResponse(newVisitSchedule, 'Visit schedule created successfully')
    );
});
...
const updateVisitSchedule = asyncHandler(async (req, res, next) => {
  ...
  const updatedVisitSchedule = await VisitSchedule.findByIdAndUpdate(
    id,
    visitScheduleUpdateData,
    {
      new: true,
      runValidators: true,
    }
  ...
  res
    ...
    .json(
      new ApiResponse(
        updatedVisitSchedule,
        'Visit schedule updated successfully'
      )
    );
});
