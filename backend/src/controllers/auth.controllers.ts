...
import { User, UserDocument } from '../models/auth.models.js';
...
const signup = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const userSignupData: IUserData = _.pick(req.body, [
      'username',
      'email',
      'password',
      'confirmPassword',
...
const login = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!username || !password) {
      ...
      const err = new ApiError(
        'Please fill in all fields (username and password)',
        422
      );
    ...
    const accessToken = jwt.sign(
      ...
      { subject: 'goatApiAccess', expiresIn: '7d' }
    );
